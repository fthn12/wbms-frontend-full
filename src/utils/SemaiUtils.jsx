...
import * as SemaiAPI from "../apis/semaiApi";

export const CopyWBToSemai = (data) => {
  data.jsonData = { ...data.jsonData, externalRefNo: data.bonTripNo };
  ...
  if (data.originWeighInKg > 0) {
    ...
    data.jsonData.originWeighInTimestamp = ConvertDateStr(data.originWeighInTimestamp);
  }
  ...
  if (data.originWeighOutKg > 0) {
    ...
    data.jsonData.originWeighOutTimestamp = ConvertDateStr(data.originWeighOutTimestamp);
  }
...
export const CopyWBRToSemai = (data) => {
  if (data.returnWeighInKg > 0) {
    ...
    data.jsonData.returnWeighInTimestamp = ConvertDateStr(data.returnWeighInTimestamp);
  }
  ...
  if (data.returnWeighOutKg > 0) {
    ...
    data.jsonData.returnWeighOutTimestamp = ConvertDateStr(data.returnWeighOutTimestamp);
  }
...
export const CopyWBDToSemai = (data) => {
  if (data.destinationWeighInKg) {
    ...
    data.jsonData.destinationWeighInOperatorName = data.destinationWeighInOperatorName;
    data.jsonData.destinationWeighInTimestamp = ConvertDateStr(data.destinationWeighInTimestamp);
  }

  if (data.destinationWeighOutKg > 0) {
    data.jsonData.destinationWeighOutKg = data.destinationWeighOutKg;
    data.jsonData.destinationWeighOutOperatorName = data.destinationWeighOutOperatorName;
    data.jsonData.destinationWeighOutTimestamp = ConvertDateStr(data.destinationWeighOutTimestamp);
  }
...
export const UpdateStatusToWBMS = (data) => {
  ...
  data.deliveryDate = data.jsonData?.deliveryDate ? moment(data.jsonData?.deliveryDate).toDate() : null;
