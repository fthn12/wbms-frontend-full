...
const EditDriver = ({ isEditOpen, onClose, dtCompanies, dtDriver }) => {
  const { useUpdateDriverMutation } = useDriver();
  const [updateDriver] = useUpdateDriverMutation();

  const handleFormSubmit = async (values, { setSubmitting, resetForm }) => {
    values.licenseED = moment(values.licenseED).toDate();
    try {
      updateDriver(values).then((results) => {
  ...
  return (
    <Dialog open={isEditOpen} fullWidth maxWidth="md">
      ...
      <Formik onSubmit={handleFormSubmit} initialValues={dtDriver} validationSchema={userSchema}>
              ...
              >
                ...
                <FormControl sx={{ gridColumn: "span 4" }}>
                  ...
                  <TextField
                    ...
                    onBlur={handleBlur}
                    onChange={handleChange}
                    value={values.licenseED ? format(new Date(values.licenseED), "yyyy-MM-dd") : ""}
                    name="licenseED"
