...
import CreateKualitasKernel from "./createKualitasKernel";
import ViewKualitasKernel from "./view";

import { useConfig, useProduct, useKualitasKernel } from "../../../hooks";

ModuleRegistry.registerModules([ClientSideRowModelModule, RangeSelectionModule, RowGroupingModule, RichSelectModule]);

const KualitasKernel = () => {
  const { MD_SOURCE } = useConfig();
  const { useGetProductsQuery, useEDispatchProductSyncMutation } = useProduct();
  const { useFindManyKualitasKernelQuery } = useKualitasKernel();

  const KualitasKernelFilter = {
    orderBy: [{ dtCreated: "desc" }],
  };

  const { data, error, refetch } = useFindManyKualitasKernelQuery(KualitasKernelFilter);
  ...
  const [columnDefs] = useState([
    ...
    //   hide: true,
    // },
    { field: "MoisturePercentage", headerName: "Moisture Percentage", flex: 1, cellStyle: { textAlign: "center" } },
    { field: "DirtPercentage", headerName: "Dirt Percentage", flex: 1, cellStyle: { textAlign: "center" } },
    { field: "StonePercentage", headerName: "Stone Percentage", flex: 1, cellStyle: { textAlign: "center" } },
    {
      field: "dtModified",
      headerName: "Tanggal",
      flex: 1,
  ...
  return (
    <Box>
      <Header title="KUALITAS KERNEL" subtitle="Kualitas Kernel" />
      ...
      {/* Create */}
      <CreateKualitasKernel isOpen={isOpen} onClose={setIsOpen} refetch={refetch} />

      {/* view */}
      <ViewKualitasKernel isViewOpen={isViewOpen} onClose={() => setIsViewOpen(false)} data={selected} refetch={refetch} />
    </Box>
  );
};

export default KualitasKernel;
