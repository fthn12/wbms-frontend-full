...
const TransactionT30CancelInView = (props) => {
  ...
  const validationSchema = yup.object().shape({
    ...
    returnWeighInRemark: yup
      ...
      .max(500, "Alasan CANCEL (PEMBATALAN) tidak boleh melebihi 500 karakter."),
  });
  ...
  const handleFormikSubmit = async (values) => {
    ...
    try {
      ...
      const response = await transactionAPI.updateById(wbTransaction.id, wbTransaction);
  ...
  return (
    <Box>
      <Header title="TRANSAKSI T30" subtitle="TIMBANG CANCEL WB-IN" />
      {openedTransaction && (
        <Formik
          // enableReinitialize
        ...
        >
          {(props) => {
            const { values, submitForm, resetForm, isValid, dirty } = props;
                      ...
                      <Grid container columnSpacing={1}>
                        ...
                        <Grid item xs={5}>
                          <Field
                            ...
                            sx={{ mt: 2, backgroundColor: "whitesmoke" }}
                            inputProps={{ readOnly: true }}
                            value={values?.returnWeighInOperatorName ? values.returnWeighInOperatorName : "-"}
                          />
                        </Grid>
                        <Grid item xs={7}>
                          <Field
                        ...
                        <Grid item xs={5}>
                          <Field
                            ...
                            sx={{ mt: 2, backgroundColor: "whitesmoke" }}
                            inputProps={{ readOnly: true }}
                            value={values?.returnWeighOutOperatorName ? values.returnWeighOutOperatorName : "-"}
                          />
                        </Grid>
                        <Grid item xs={7}>
                          <Field
...
export default TransactionT30CancelInView;
