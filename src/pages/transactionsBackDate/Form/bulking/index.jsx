...
import Reject from "./dispatch/reject";
...
const PksManualEntryWBIn = () => {
  ...
  const handleDispatchNormal = async (values) => {
    ...
    try {
      ...
      tempTrans.dtTransaction = moment(tempTrans.destinationWeighOutTimestamp)
  ...
  const handleDispatchReject = async (values) => {
    let tempTrans = { ...values };
    ...
    try {
      ...
      toast.success(`Transaksi Reject Bulking Bakdate telah tersimpan.`);
    } catch (error) {
      return toast.error(`${error.message}.`);
    }
  };
  ...
  return (
    <Box>
      <Header title="TRANSAKSI BULKING" subtitle="Transaksi BackDate Form" />
      {wbTransaction && (
        <Formik
          enableReinitialize
          onSubmit={(values) => {
        ...
        >
          {(props) => {
            ...
            const handleDspRjtSubmit = (normalReason) => {
              if (normalReason.trim().length <= 10)
                return toast.error(
                  "Alasan (MANUAL ENTRY) harus melebihi 10 karakter."
                );
              ...
              <Form>
                ...
                <Paper sx={{ mt: 1, p: 2, minHeight: "71.5vh" }}>
                  <Grid container spacing={2}>
                    ...
                    {selectedStatus === 4 && (
                      <Reject setFieldValue={setFieldValue} values={values} />
                    )}
