...
import RejectBulking from "./dispatch/rejectBulking";
import RejectT300 from "./dispatch/rejectT300";
import TBS from "./tbs";
...
const PksManualEntryWBIn = () => {
  ...
  const handleDispatchRjtt300 = async (values) => {
    let tempTrans = { ...values };
    ...
    try {
      ...
      setIsLoading(true);
      const response =
        await transactionAPI.ManualEntryBackDateRejectT300Dispatch(tempTrans);
      ...
      toast.success(`Transaksi Reject T300 BACKDATE telah tersimpan.`);
    } catch (error) {
      return toast.error(`${error.message}.`);
    }
  };

  const handleDispatchRjtBulking = async (values) => {
    let tempTrans = { ...values };
    ...
    try {
      ...
      setIsLoading(true);
      const response =
        await transactionAPI.ManualEntryBackDateRejectBulkingDispatch(
          tempTrans
        );
      ...
      toast.success(`Transaksi Reject Bulking BACKDATE telah tersimpan.`);
    } catch (error) {
      return toast.error(`${error.message}.`);
    }
  };
  ...
  const handleTbsSubmit = async (values) => {
    ...
    try {
      ...
      const response = await transactionAPI.ManualEntryBackDateTbs(tempTrans);
  ...
  const handleKernelSubmit = async (values) => {
    ...
    try {
      ...
      const response = await transactionAPI.ManualEntryBackDateKernel(
        tempTrans
      );
  ...
  return (
    <Box>
      <Header
        title="Transaksi PKS BackDate"
        subtitle="Transaksi BackDate Form"
      />
      {wbTransaction && (
        <Formik
          enableReinitialize
          onSubmit={(values) => {
            if (selectedStatus === 1) {
              ...
              handleDispatchRjtt300(values);
            } else if (selectedStatus === 4) {
              handleDispatchRjtBulking(values);
        ...
        >
          {(props) => {
            ...
            const handleKrlSubmit = (normalReason) => {
              if (normalReason.trim().length <= 10)
                return toast.error(
                  "Alasan (BACKDATE) harus melebihi 10 karakter."
                );
            ...
            const handleTbsSubmit = (normalReason) => {
              if (normalReason.trim().length <= 10)
                return toast.error(
                  "Alasan (BACKDATE) harus melebihi 10 karakter."
                );
            ...
            const handleDispatchRjtt300 = (normalReason) => {
              if (normalReason.trim().length <= 10)
                return toast.error(
            ...
            const handleDispatchRjtBulking = (normalReason) => {
              if (normalReason.trim().length <= 10)
                return toast.error(
                  "Alasan REJECT (PENGEMBALIAN) harus melebihi 10 karakter."
                );
                ...
                <Box sx={{ display: "flex", mt: 3, justifyContent: "end" }}>
                  ...
                  {selectedStatus === 4 && (
                    <CancelConfirmation
                      ...
                      caption="SIMPAN"
                      content="Anda yakin melakukan REJECT (PENGEMBALIAN) transaksi WB ini? Berikan keterangan yang cukup."
                      onClose={handleDispatchRjtBulking}
                      isDisabled={
                        !(
                          isValid &&
                          dirty &&
                  ...
                  <Grid container spacing={2}>
                    <Grid item xs={12} sm={6} lg={3}>
                      ...
                      {selectedOption === 1 && (
                        ...
                        >
                          {dtProgresStatus &&
                            dtProgresStatus.map((data, index) => (
                              <MenuItem key={index} value={data.id}>
                                {data.value}
                              </MenuItem>
                            ))}
